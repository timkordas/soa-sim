...
if __name__ == '__main__':
    ...
    brancher1 = PQueueBrancher(env, completers)
    brancher2 = PQueueBrancher(env, completers)
    gen1.out = brancher1
    gen2.out = brancher2
    env.run(until=25000)

    for i in range(n_completers):
        print ("{} reqs: {} hiwater {} inflight {}".format(i, completers[i].requests_rec, completers[i].hiwat, brancher1.inflight[i]))
